...
export const ProductLabelPrintSettings: React.FC<{
  ...
  return <><style>{`@media print { @page { size: ${pageSize}; margin: 0; } }`}</style><div className="no-print flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-white p-3"><label className="flex h-10 items-center gap-2 text-sm font-medium"><input type="checkbox" checked={!dimensions.autoFit} onChange={(event) => setAutoFit(!event.target.checked)} />Manual label size</label>{!dimensions.autoFit && <><NumberSetting label="Width mm" value={dimensions.widthMm} onChange={(value) => change('widthMm', value)} /><NumberSetting label="Height mm" value={dimensions.heightMm} onChange={(value) => change('heightMm', value)} /></>}<label className="flex h-10 items-center gap-2 text-sm font-medium"><input type="checkbox" checked={showPrice} onChange={(event) => onShowPriceChange(event.target.checked)} />Show selling price</label><label className="flex h-10 items-center gap-2 text-sm font-medium"><input type="checkbox" checked={showCode} onChange={(event) => onShowCodeChange(event.target.checked)} />Price code in SKU</label><p className="basis-full text-xs text-slate-500">Auto fit expands the label for its content. Use manual size only when matching fixed sticker stock.</p></div></>;
};
