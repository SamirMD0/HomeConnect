  ...
  return <section className="rounded-lg border border-slate-200 bg-slate-50">
    ...
    {open && <div className="grid gap-5 border-t border-slate-200 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(300px,0.8fr)]">
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2"><button type="button" onClick={submit} disabled={save.isPending} className="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white"><Save className="h-4 w-4" />Save Pricing / حفظ التسعير</button>{preview.data?.cashPrice && preview.data.cashPrice !== product.price && <button type="button" onClick={copyCalculated} className="inline-flex items-center gap-2 rounded-lg border border-brand-300 px-4 py-2 text-sm font-semibold text-brand-700"><Copy className="h-4 w-4" />Use calculated price / اعتماد السعر المحسوب</button>}</div>
      </div>
