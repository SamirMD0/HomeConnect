...
export const ProductMobileCard: React.FC<ProductMobileCardProps> = ({
  product, selected, canAdmin, onSelect, onView, onEdit, onArchive, onRestore,
}) => {
  ...
  return (
    <article className={`rounded-lg border bg-white shadow-sm ${
      selected ? 'border-emerald-300 ring-1 ring-emerald-200' : product.isActive ? 'border-slate-200' : 'border-slate-300 bg-slate-50'
    }`}>
      <div className="flex items-start justify-between gap-3 p-4">
        <label className="flex min-w-0 items-start gap-3">
          ...
          <ProductImageView
            ...
            image={product.image}
            alt=""
            className="h-12 w-12 shrink-0 rounded-md border border-slate-200"
          />
          <span className="min-w-0">
            <span className="user-text block truncate font-semibold text-slate-900" dir="auto">{product.name}</span>
            <span className="user-text block truncate text-sm text-slate-500" dir="auto">
      ...
      <div className="border-t border-slate-100 px-4 py-3">
        ...
        <div className="flex items-baseline justify-between gap-3">
          ...
          <span dir="ltr" className="text-lg font-bold tabular-nums text-emerald-700">
            {preview ? formatMoney(preview.cashPrice) : product.price ? formatMoney(product.price) : '—'}
          </span>
        </div>

        {manualDiffers && (
          <p className="mt-1 text-right text-[11px] text-amber-600">
            manual / يدوي <span dir="ltr" className="tabular-nums">{formatMoney(product.price as string)}</span>
          </p>
        )}
        {!preview && product.price && (
          <p className="mt-1 text-right text-[11px] text-slate-400">manual price only / سعر يدوي فقط</p>
        )}

        {preview?.installmentPrice && (
          <dl className="mt-3 space-y-1.5 border-t border-dashed border-slate-200 pt-3">
            ...
            {preview.monthlyPayment && (
              <Row
                label={preview.installmentMonths ? `Monthly × ${preview.installmentMonths} / القسط الشهري` : 'Monthly / القسط الشهري'}
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-dashed border-slate-200 pt-3">
          {pricing?.useCustomPricing
            ? <span className="rounded bg-blue-50 px-2 py-0.5 text-[11px] font-medium text-blue-700">Custom / مخصص</span>
            : pricing?.presetName
              ? <span dir="auto" className="max-w-full truncate rounded bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-600">{pricing.presetName}</span>
              : <span className="text-[11px] text-slate-400">No formula / دون صيغة</span>}
          {canAdmin && pricing?.costPrice && (
            <span className="ms-auto text-[11px] text-slate-500">
              Cost / التكلفة <strong dir="ltr" className="font-semibold tabular-nums text-slate-700">{formatMoney(pricing.costPrice)}</strong>
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 px-4 py-3">
        <button type="button" onClick={onView} className="inline-flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-700">
          <Eye className="h-4 w-4" />Details / التفاصيل
        </button>
        <IconButton label="Edit product / تعديل المنتج" onClick={onEdit}><Edit3 className="h-4 w-4" /></IconButton>
        <Link to={`/products/${product.id}/label`} title="Print label / طباعة الملصق" aria-label="Print label / طباعة الملصق" className="rounded-lg border border-slate-300 p-2 text-slate-600 hover:bg-slate-100">
...
const Row: React.FC<{ label: string; value: string; strong?: boolean }> = ({ label, value, strong }) => (
  <div className="flex items-baseline justify-between gap-3 text-xs">
    ...
    <dd dir="ltr" className={`shrink-0 tabular-nums ${strong ? 'font-semibold text-slate-900' : 'font-medium text-slate-700'}`}>{value}</dd>
  </div>
);

const IconButton: React.FC<{ label: string; onClick: () => void; children: React.ReactNode }> = ({ label, onClick, children }) => (
  <button type="button" title={label} aria-label={label} onClick={onClick} className="rounded-lg border border-slate-300 p-2 text-slate-600 hover:bg-slate-100">
    {children}
  </button>
);
