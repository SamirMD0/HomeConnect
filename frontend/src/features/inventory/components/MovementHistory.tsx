...
export const MovementHistory: React.FC<{ movements: StockMovement[]; emptyLabel?: string }> = ({ movements, emptyLabel = 'No stock movements / لا توجد حركات مخزون' }) => movements.length
  ? <div className="divide-y divide-slate-100">{movements.map((movement) => { const sourceOrder = movement.salesFulfillmentMovement?.salesOrder ?? movement.salesFulfillmentReversalMovement?.salesOrder; const receipt = movement.receivingMetadata; return <article key={movement.id} className="py-3 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2"><strong>{movementTypeLabel(movement.movementType)}</strong><time className="text-xs text-slate-500">{new Date(movement.createdAt).toLocaleString()}</time></div>
      ...
      {receipt && <div className="mt-1 text-xs"><p className="text-slate-500">{receipt.supplierName ?? 'No supplier / بدون مورّد'}{receipt.referenceNumber ? ` · ${receipt.referenceNumber}` : ''} · {receipt.receivedOn}</p><Link className="inline-block font-semibold text-emerald-700 hover:underline" to={`/inventory/receiving/${receipt.receivingId}`}>View receiving / عرض مستند الإدخال</Link></div>}
    </article>;})}</div>
