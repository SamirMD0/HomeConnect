      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            <option value="ONE_TIME">One-Time Debts</option>
          ...
          <tbody className="divide-y divide-slate-200 bg-white">
            {filtered.map((transaction) => {
              const amountColor = 
                ...
                (transaction.type === 'ONE_TIME' || transaction.type === 'INSTALLMENT') ? 'text-red-600' : 'text-slate-600';
              ...
              const totalPaid = transaction.payments?.filter((p: any) => p.type === 'PAYMENT').reduce((sum: number, p: any) => sum + Number(p.amount), 0) || 0;
              ...
              return (
                ...
                <tr className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="whitespace-nowrap px-3 py-4 text-sm font-medium">
                    <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${
                      (transaction.type === 'ONE_TIME' || transaction.type === 'INSTALLMENT') ? 'bg-red-50 text-red-700 ring-red-600/10' :
                  ...
                  <td className={`whitespace-nowrap px-3 py-4 text-sm font-medium text-right ${amountColor}`}>
                    <div className="flex flex-col items-end">
                      ...
                      {(transaction.type === 'ONE_TIME' || transaction.type === 'INSTALLMENT') && totalPaid > 0 && (
                  ...
                  <td className="whitespace-nowrap px-3 py-4 text-sm font-semibold text-slate-900 text-right">
                    ...
                    {(transaction.type === 'ONE_TIME' || transaction.type === 'INSTALLMENT') && onPayDebt && (
                      <div className="mt-2">
                        {remaining <= 0 ? (
                ...
                {/* Nested Payments and Schedules */}
                {transaction.payments && transaction.payments.length > 0 && transaction.payments.map((child: any) => {
                  const pDateObj = new Date(child.date);
                  const pDateStr = pDateObj.toLocaleDateString('en-GB');
                  
                  const isPayment = child.type === 'PAYMENT';
                  
                  return (
                    <tr key={child.id} className="bg-slate-50/50 text-sm border-t-0">
                      <td className="whitespace-nowrap py-3 pl-4 pr-3 text-sm text-slate-500 sm:pl-10 border-t-0">
                        <span className="text-slate-300 mr-2">↳</span>
                        {isPayment ? pDateStr : `Due: ${new Date(child.dueDate).toLocaleDateString('en-GB')}`}
                      </td>
                      ...
                      <td className={`whitespace-nowrap px-3 py-3 text-sm font-medium text-right border-t-0 ${isPayment ? 'text-emerald-600' : 'text-slate-500'}`}>
                        {isPayment ? '-' : ''}{Number(child.amount).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
                      </td>
