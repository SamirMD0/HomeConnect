...
export const serviceJobsApi = {
  ...
  status: async (id: string, input: { status: ServiceJobStatus; reason?: string; accountPassword?: string; sentToCompanyDate?: string | null; receivedFromCompanyDate?: string | null; returnedToCustomerDate?: string | null }): Promise<ServiceJob> => (await api.post(`/service-jobs/${id}/status`, input)).data.data,
