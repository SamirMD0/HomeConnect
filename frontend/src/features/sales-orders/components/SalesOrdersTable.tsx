...
export function SalesOrdersTable({ orders }: { orders: SalesOrder[] }) {
  ...
  const columns = [
    ...
    { header: 'Total', accessor: (order: SalesOrder) => <span className="font-semibold tabular-nums">{formatMoney(order.totalAmount, order.currency)}</span> },
