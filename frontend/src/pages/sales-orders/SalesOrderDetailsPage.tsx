import { CreditLimitWarning, useCreditLimitWarning } from '../../features/customer-financial/components/CreditLimitWarning';
...
export function SalesOrderDetailsPage() {
  ...
  const close = () => { credit.reset(); receiptKey.current = crypto.randomUUID(); setDialog(null); setReason(''); setPassword(''); setDueDate(''); setSelectedItem(null); setLine(emptySalesLine()); };
  const run = async () => {
    try {
      ...
      if (dialog === 'debt') await debtMutation.mutateAsync({ dueDate, ...credit.payload });
      ...
      if (dialog === 'payment') await paymentMutation.mutateAsync({ idempotencyKey: receiptKey.current, paidAmount, debtDueDate: dueDate || null, reason, accountPassword: password, ...credit.payload });
      if (dialog === 'edit-details') await updateMutation.mutateAsync({ notes: notes || null });
      if (dialog === 'add-item') await addItemMutation.mutateAsync({ ...line, ...credit.payload, ...(dueDate ? { debtDueDate: dueDate } : {}), ...(order.fulfillmentStatus !== 'DRAFT' ? { reason, accountPassword: password } : {}), ...credit.payload });
      if (dialog === 'edit-item' && selectedItem) await itemMutation.mutateAsync({ itemId: selectedItem.id, data: { ...line, ...credit.payload, ...(dueDate ? { debtDueDate: dueDate } : {}), ...(order.fulfillmentStatus !== 'DRAFT' ? { reason, accountPassword: password } : {}) } });
      if (dialog === 'remove-item' && selectedItem) await removeItemMutation.mutateAsync({ itemId: selectedItem.id, data: order.fulfillmentStatus === 'DRAFT' ? credit.payload : { ...(dueDate ? { debtDueDate: dueDate } : {}), reason, accountPassword: password, ...credit.payload } });
      toast.success('Sales order updated'); close();
    } catch (error) { if (!credit.capture(error)) toast.error('Unable to update sales order'); }
  };
