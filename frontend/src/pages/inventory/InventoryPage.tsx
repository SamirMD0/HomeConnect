...
import { AlertTriangle, PackagePlus, PackageSearch, PackageX, ScanLine, Search, Warehouse } from 'lucide-react';
...
export const InventoryPage: React.FC = () => {
  ...
  return <div className="space-y-6"><header className="flex flex-wrap items-end justify-between gap-3"><div><div className="flex items-center gap-2"><Warehouse className="h-7 w-7 text-emerald-700" /><h1 className="text-2xl font-bold">Inventory / المخزون</h1></div><p className="mt-1 text-sm text-slate-500">Every quantity change has a reason and history / لكل تغيير في الكمية سبب وسجل</p></div><div className="flex flex-wrap gap-2"><Link to="/inventory/receiving" className="rounded-lg border border-emerald-600 px-4 py-2.5 text-sm font-semibold text-emerald-700">Receiving history / سجل الإدخال</Link><Link to="/inventory/receiving/new" className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white"><PackagePlus className="h-4 w-4" />Receive Stock / إدخال إلى المخزون</Link></div></header>
