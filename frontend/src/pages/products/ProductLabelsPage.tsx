...
export const ProductLabelsPage: React.FC = () => {
  ...
  const [showCode, setShowCode] = useState(true); const [showPrice, setShowPrice] = useState(true); const [dimensions, setDimensions] = useState(loadProductLabelDimensions);
  const labels = useQueries({ queries: ids.map((id) => ({ queryKey: productKeys.label(id, showCode, showPrice), queryFn: () => productsApi.label(id, showCode, showPrice), retry: false })) });
  ...
  return <div className="product-label-page space-y-5">
    ...
    <ProductLabelPrintSettings dimensions={dimensions} onChange={setDimensions} showCode={showCode} onShowCodeChange={setShowCode} showPrice={showPrice} onShowPriceChange={setShowPrice} />
    ...
    <div className={`product-label-grid ${dimensions.autoFit ? 'product-label-grid-auto' : ''}`} style={{ '--label-width': `${dimensions.widthMm}mm` } as React.CSSProperties}>{ready.map((product) => <ProductLabel key={product.id} product={product} dimensions={dimensions} />)}</div>
  </div>;
};
