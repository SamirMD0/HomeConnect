...
export const ProductLabelsPage: React.FC = () => {
  ...
  const labels = useQueries({ queries: ids.map((id) => ({ queryKey: productKeys.label(id), queryFn: () => productsApi.label(id), retry: false })) });
  ...
  return <div className="product-label-page space-y-5">
    ...
    <div className="product-label-grid">{ready.map((product) => <ProductLabel key={product.id} product={product} />)}</div>
  </div>;
};
