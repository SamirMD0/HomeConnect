...
export class DashboardActivityService {
  ...
  static aggregate(records: DashboardActivityRecords, limit = 15): DashboardActivityData {
    const items: DashboardActivityItem[] = [
      ...
      ...records.payments.map((row) => ({ id: `payment:${row.id}`, module: 'payments' as const, action: 'PAYMENT_RECORDED', entityId: row.id, title: `Payment recorded for ${row.customer?.name ?? 'Walk-in / زبون عابر'}`, amount: moneyToApiString(row.totalAmount, row.currency), occurredAt: row.createdAt.toISOString(), actor: row.createdBy.fullName, route: '/ledger?view=payments' })),
