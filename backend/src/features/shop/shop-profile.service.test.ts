...
describe('shop profile service', () => {
  ...
  it('audits detail mutations without verifying an account password', async () => {
    await ShopProfileService.updateShopProfile({ name: 'Home Connect Beirut' }, user, context);
  ...
  it('connects the default template relation instead of writing the foreign key scalar', async () => {
    ...
    await ShopProfileService.updateShopProfile({ defaultPricingCardTemplateId: templateId }, user, context);
  ...
  it('disconnects the default template relation when the template id is cleared', async () => {
    await ShopProfileService.updateShopProfile({ defaultPricingCardTemplateId: null }, user, context);
  ...
  it('stores logo bytes and writes an audit, with no password verification', async () => {
    ...
    const result = await ShopProfileService.updateShopProfileLogo(bytes, 'image/webp', user, context);
