...
v2.1.0 adds a category-agnostic **pricing card** renderer alongside the legacy label — one template config drives modern shelf cards for TVs, washing machines, refrigerators, small appliances, and trimmers from the same product data. Templates, feature icons, brand logos, and the company logo are all admin-managed; a shop-wide rollout mode picks whether the legacy label, the new pricing card, or both appear in the print flow. See the [v2.1.0 release notes](docs/phases/Versions/phase-2-1-0/RELEASE_NOTES_V2_1_0.md).
