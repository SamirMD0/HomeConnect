...
- `JWT_SECRET` has a hard-coded fallback in `auth.service.ts` and `auth.middleware.ts`; this is a security risk documented in `docs/FINANCIAL_FLOW_AUDIT.md`, but changing it may break local startup unless environment setup is addressed first. Phase 1 will document the risk and avoid broad auth behavior changes.
