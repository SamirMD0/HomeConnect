...
1. **Schema and migration drift** noted in `docs/FINANCIAL_FLOW_AUDIT.md` section 14, between `20260723094305_init_ledger` and the current schema. Inspect `_prisma_migrations` state on the business machine before applying the additive migration. Do not resolve drift by resetting.
...
docs/PROJECT_ROADMAP.md      (checkpoint tracking)
docs/PHASE_12_FINANCIAL_CORRECTIONS_DESIGN.md  (this document)
```
