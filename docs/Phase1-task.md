...
- [x] Verify `npm run dev` starts Vite dev server
...
- [x] Verify Tailwind classes render in the placeholder page
...
- [x] Verify `npm run lint` and `npm run format` work
...
- [x] Verify Express starts and responds to health check
...
- [x] Verify migration creates table in PostgreSQL
...
- [x] Configure Electron to load Vite dev server in development
- [x] Configure Electron to load built files in production
- [x] Add `npm run dev:electron` script
- [x] Verify Electron launches and displays the React placeholder page
...
- [x] Create `feature/phase-1-foundation` branch

---

## Verification (Phase 1 Complete When)
- [x] `npm run dev` starts Vite + Express concurrently
- [x] `npm run dev:electron` launches Electron with React app inside
- [x] `GET http://localhost:3001/api/v1/health` returns `200 OK`
- [x] Prisma is connected to PostgreSQL with `users` table
- [x] ESLint and Prettier run clean
- [x] Git repository has clean history on `feature/phase-1-foundation`
